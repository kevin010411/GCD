...
class WTConv3d(nn.Module):
    def __init__(
        ...
        # 建立 3D Filters
        self.wt_filter, self.iwt_filter = create_wavelet_filter_3d(
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels):
            ...
            curr_x_tag = curr_x.reshape(
                shape_x[0], shape_x[1] * 8, shape_x[3], shape_x[4], shape_x[5]
            )
            curr_x_tag = self.wavelet_scale[i](self.wavelet_convs[i](curr_x_tag))
            curr_x_tag = curr_x_tag.reshape(shape_x)

            x_ll_in_levels.append(curr_x_tag[:, :, 0, :, :, :])
            x_h_in_levels.append(
                curr_x_tag[:, :, 1:8, :, :, :]
            )  # Remaining 7 high-freq bands
        ...
        for i in range(self.wt_levels - 1, -1, -1):
            ...
            next_x_ll = next_x_ll[
                :, :, : curr_shape[2], : curr_shape[3], : curr_shape[4]
            ]
...
class UWTNet(nn.Module):
    def __init__(
        self, in_channels, out_channels, wave_level, model_size, deep_supervision=True
    ):
        ...
        # Decoder
        self.decoder4 = self.dconv_block(
            num_channels[4] * 2, num_channels[3], wave_level
        )
        self.decoder3 = self.dconv_block(
            num_channels[3] * 2, num_channels[2], wave_level
        )
        self.decoder2 = self.dconv_block(
            num_channels[2] * 2, num_channels[1], wave_level
        )
        self.decoder1 = self.dconv_block(
            num_channels[1] * 2, num_channels[0], wave_level
        )
    ...
    def forward(self, x):
        ...
        up_middle = F.interpolate(
            middle, scale_factor=2, mode="trilinear", align_corners=False
        )
        # Pad if necessary (for odd dimensions)
        if up_middle.shape[2:] != enc4.shape[2:]:
            up_middle = F.interpolate(
                up_middle, size=enc4.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        # Level 3
        up_dec4 = F.interpolate(
            dec4, scale_factor=2, mode="trilinear", align_corners=False
        )
        if up_dec4.shape[2:] != enc3.shape[2:]:
            up_dec4 = F.interpolate(
                up_dec4, size=enc3.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        # Level 2
        up_dec3 = F.interpolate(
            dec3, scale_factor=2, mode="trilinear", align_corners=False
        )
        if up_dec3.shape[2:] != enc2.shape[2:]:
            up_dec3 = F.interpolate(
                up_dec3, size=enc2.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        # Level 1
        up_dec2 = F.interpolate(
            dec2, scale_factor=2, mode="trilinear", align_corners=False
        )
        if up_dec2.shape[2:] != enc1.shape[2:]:
            up_dec2 = F.interpolate(
                up_dec2, size=enc1.shape[2:], mode="trilinear", align_corners=False
            )
        ...
        final_feat = F.interpolate(
            dec1, scale_factor=2, mode="trilinear", align_corners=False
        )
