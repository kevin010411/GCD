...
from ..uwtnet_3d import WTConv3d
...
class W_InceptionBlock(nn.Module):
    ...
    def __init__(self, dim, kernel_size, exp_rate, drop_path, wave_level=1):
        ...
        self.spatial_branch = InceptionNeXtBlock_V2(
            dim=dim, hwd_kernel_size=kernel_size, exp_rate=exp_rate, drop_path=drop_path
        )
...
class WISNET(nn.Module):
    def __init__(
        ...
        wave_level=2,  # 小波層數 0表示decoder不使用小波修復，None則是encoder和decoder都不使用小波修復
        **kwargs,
    ) -> None:
        super().__init__()
        ...
        if wave_level is not None and wave_level > 0:
            self.wt_dec4 = WTConv3d(
                feature_sizes[2], feature_sizes[2], wt_levels=wave_level
            )
            self.wt_dec3 = WTConv3d(
                feature_sizes[1], feature_sizes[1], wt_levels=wave_level
            )
            self.wt_dec2 = WTConv3d(
                feature_sizes[0], feature_sizes[0], wt_levels=wave_level
            )
            # 最後一層通常不做 WT，直接輸出
