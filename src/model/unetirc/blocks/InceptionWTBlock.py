...
class InceptionWTBlock(nn.Module):
    def __init__(
        self, dim, kernel_size=11, exp_rate=4, drop_path=0.0, wt_levels=1, wt_type="db1"
    ):
        super().__init__()
        self.gc = int(dim * 0.2)
        ...
        self.dwconv_hwd = nn.Conv3d(
            self.gc, self.gc, kernel_size=5, padding=2, groups=self.gc
        )
        ...
        self.dwconv_d = nn.Conv3d(
            ...
            padding=(kernel_size // 2, 0, kernel_size // 2),
            groups=self.gc,
        )
