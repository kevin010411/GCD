...
def _strip_in_running_stats(sd, model):
    """
    將 state_dict 中屬於 InstanceNorm3d 且 track_running_stats=False 的
    running_mean / running_var 先移除，避免不相容。
    """
    # 找出所有不追蹤 running stats 的 InstanceNorm3d 的路徑
    in_paths = {
        name
        for name, m in model.named_modules()
        if isinstance(m, nn.InstanceNorm3d)
        and not getattr(m, "track_running_stats", False)
    }

    drop = []
    for p in in_paths:
        for suf in ("running_mean", "running_var"):
            k = f"{p}.{suf}" if p else suf
            if k in sd:
                drop.append(k)

    # 一些舊權重也可能帶有 num_batches_tracked（穩健起見一併清）
    for p in in_paths:
        k = f"{p}.num_batches_tracked" if p else "num_batches_tracked"
        if k in sd:
            drop.append(k)
    ...
    for k in set(drop):
        sd.pop(k, None)
    if drop:
        print(f"info: stripped {len(set(drop))} InstanceNorm running-stat keys")
    return sd
