...
class MainWindowPresenter:
    def __init__(
        ...
        self.perturbation_preview = PerturbationPreviewAdapter(
            ...
            render_current_items=self._render_current_items,
            set_data_controls_enabled=self._set_data_controls_enabled,
            set_camera_controls_enabled=self._set_camera_controls_enabled,
            selected_dataset_id=lambda: self.selected_perturbation_dataset_id,
        )
        ...
        *,
        ...
        def _compute(progress_callback=None):
            ...
            if family_id == "perturbation" and callable(progress_callback):
                ...
                active_method_params.update(
                    self.perturbation_preview.runtime_method_params(progress_callback)
                )
    ...
    def on_xai_run_requested(self, family_id: str) -> None:
        ...
        if family_id == "perturbation":
            self.perturbation_preview.begin_if_enabled(
                self._selected_perturbation_preview_enabled()
            )
            self._sync_perturbation_preview_state()
    ...
    def _sync_perturbation_preview_state(self) -> None:
        self._perturb_preview_active = self.perturbation_preview.active
        self._perturb_preview_enabled = self.perturbation_preview.enabled
        self._perturb_pause_controller = self.perturbation_preview.pause_controller
        self._pre_perturb_preview_rotation_running = (
            self.perturbation_preview._pre_preview_rotation_running
        )

    def on_perturbation_preview_pause_toggled(self, paused: bool) -> None:
        self.perturbation_preview.pause_toggled(bool(paused))
        self._sync_perturbation_preview_state()
    ...
    def _prepare_camera_for_perturbation_preview(self) -> None:
        self.perturbation_preview._prepare_camera()
        self._sync_perturbation_preview_state()

    def _restore_camera_after_perturbation_preview(self) -> None:
        self.perturbation_preview._restore_camera()
        self._sync_perturbation_preview_state()

    def _camera_control_allowed(self) -> bool:
        return self.perturbation_preview.camera_control_allowed()

    def _show_perturbation_preview(self, payload: dict[str, object]) -> None:
        self.perturbation_preview.handle_payload(payload)
        self._sync_perturbation_preview_state()
    ...
    def _update_perturbation_preview_volume(
        ...
        return self.perturbation_preview._update_preview_volume(
            data, spacing, metadata, colors, opacities
        )

    def _perturbation_input_transfer_function(self) -> TransferFunction:
        return self.perturbation_preview._input_transfer_function()

    def _set_preview_box(self, preview_box: object) -> bool:
        return self.perturbation_preview._set_preview_box(preview_box)

    @staticmethod
    def _normalized_preview_box(preview_box: object):
        return PerturbationPreviewAdapter.normalized_preview_box(preview_box)

    def _finish_perturbation_preview(self) -> None:
        self.perturbation_preview.finish()
        self._sync_perturbation_preview_state()
